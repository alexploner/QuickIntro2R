...
# We assume that the data files are in the same directory as the script
library(readxl)
...
# the list of t-tests
tstat <- unlist( lapply(tttests, function(x) x$statistic))
pval  <- unlist( lapply(tttests, function(x) x$p.value))
